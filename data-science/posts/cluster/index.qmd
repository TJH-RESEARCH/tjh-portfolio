...
plot_penguins <-
  ...
  theme(
    ...
    plot.caption = element_text(size = 14, hjust = -.01),
...
While we cannot [easily visualize past two or three dimensions](https://www.data-to-viz.com/caveat/3d.html), mathematically we can use the same unsupervised learning tools in as many dimensions as we want. Instead of using human eyes, unsupervised learning algorithms try to find the clusters mathematically.
...
plot_penguins + 
  geom_point(data = penguins, aes(color = cluster, shape = correct)) + labs(
    title = "Did the clusters find the correct groups?",
...
-   How many clusters should I ask the model to find? Compare the fit of different solutions.
...
We will start with the elbow method. 
...
The silhouette method is similar to the elbow method. 
...
If the gap statistic suggests 1 cluster. That isn't really a cluster now is it. Basically, this is an inability to reject the null hypothesis that the data clusters into latent groups at all. 
...
Going by those results, the 2-group and 3- group solution look the best. Let's limit the number of clusters to compare head-to-head just those 2- and 3-group solutions.
